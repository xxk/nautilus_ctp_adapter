...
fn main() {
    ...
    let repo_root = manifest_dir
        ...
        .to_path_buf();
    let synced_from = repo_root.join("vendor").join("ctp").join("bin").join("_synced_from.txt");
    println!("cargo:rerun-if-changed={}", synced_from.display());

    let Some(sdk_dir) = locate_sdk_dir(&repo_root, &synced_from) else {
        println!("cargo:warning=CTP SDK not found; building scaffold-only repo-owned ctp_native bridge");
        return;
    };
    ...
    println!("cargo:warning=using CTP SDK directory {}", sdk_dir.display());
}

...
1. [ctp-live-connectivity](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/ctp-live-connectivity.md)
2. [nautilus-instrument-provider](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/nautilus-instrument-provider.md)
3. [nautilus-live-marketdata](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/nautilus-live-marketdata.md)
4. [nautilus-live-execution](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/nautilus-live-execution.md)
5. [live-ops-and-reconciliation](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/live-ops-and-reconciliation.md)

## 十一、AI-TASK-QUEUE

**当前活动 Topic**：初版 Topic 1-5 已完成；当前扩展 frontier 为 `live-session-order-query-hardening`

- [x] `docs/topics/ctp-live-connectivity.md`
- [x] `docs/topics/nautilus-instrument-provider.md`
- [x] `docs/topics/nautilus-live-marketdata.md`
- [x] `docs/topics/nautilus-live-execution.md`
- [x] `docs/topics/live-ops-and-reconciliation.md`

**当前 next action**：当前扩展主线已切到 [live-session-order-query-hardening](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/live-session-order-query-hardening.md)，当前 active change 为 [20260409__live-session-order-query-hardening__offhours-query-snapshot-hardening](/D:/Nautilus/nautilus_ctp_adapter/docs/changes/20260409__live-session-order-query-hardening__offhours-query-snapshot-hardening/plan.md)。继续非交易查询开发时，应先运行 `python scripts/check_rust_gate.py`；若 blocker 仍稳定为 `ctp_vendor_bridge-scaffold-only`，则在 C3 收口后再切入 U1 的 SDK/live DLL handoff。
...
1. [CTP live connectivity roadmap](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/ctp-live-connectivity.md)
...
若继续围绕真实账户 `025292` 做功能开发，当前扩展状态如下：

1. [live-session-order-query-hardening](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/live-session-order-query-hardening.md) 已进入进行中，当前聚焦非交易时段 `query / reconciliation / merged policy` 开发面。
2. [live-ops-truth-snapshot](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/live-ops-truth-snapshot.md) 当前已转为阻塞状态，原因是 disconnect storm 挂起。
3. [rust-ctp-runtime-cutover](/D:/Nautilus/nautilus_ctp_adapter/docs/topics/rust-ctp-runtime-cutover.md) 已完成，不再是当前 active frontier。

推荐原因：

1. `live-session-order-query-hardening` 直接承接当前真实可推进的 session-window 能力面。
2. 它可以先推进 offhours read-only 路径，不新增交易风险。
3. 当前真正的外部 blocker 已经被 `check_rust_gate.py` 前置暴露，不再需要反复从 auth/front 假设入手。

对于 `rust-ctp-runtime-cutover`，补充说明：

1. 它不是新的业务功能 topic，而是 ownership / runtime path 的二期切换 topic。
2. 该 topic 已完成，当前留下的是 live vendor bridge 外部依赖，而不是 cutover 本身未完成。
3. 它的正式目标已经达成：主线路径已收敛到 `Python adapter -> PyO3 bridge -> Rust runtime -> repo-owned ctp_native -> CTP vendor DLL`。

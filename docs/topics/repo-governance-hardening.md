...
**domain**：repo_governance
**用途**：修复 Topic 1→2→3 推进后治理层出现的 4 个陈旧点，建立 AGENTS.md 保持鲜活的切换规则，升级 topic 索引，补齐最小文档门禁脚本，并沉淀 topic 切换 checklist；随后补齐 Rust 校验正式门禁。
...
2. `docs/topics/README.md` 只列了 2 个 topic，遗漏了 `nautilus-instrument-provider` 和 `nautilus-live-marketdata`，且无 current state 信息。
...
| C2 | `20260402__governance-harness__changes-topic-index-upgrade` | 升级 `docs/topics/README.md`：补全 topic 列表，加入 current state 信息 | ✅ 已完成 |
...
- [x] 完成 C6：dslresearch-frontier-alignment
...
2. `docs/topics/README.md` 列出所有 6 个 topic（含本 governance topic），显示 current state。
...
5. 当前仓已引入 machine-readable topic registry、同步式 topic index、current frontier CLI 与一键 topic governance check。

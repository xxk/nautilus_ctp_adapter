...
| --- | --- | --- | --- | --- | --- | --- |
| A1 | Success 1: smoke 最小入口冻结 | 检查 scripts/README/plan | 有唯一 smoke 入口 | 入口清晰唯一 | 存在多个 competing 入口 | 当前 change |
| A2 | Success 2: 最小成功信号冻结 | 检查 acceptance | “何为通过”被明确写清 | 成功信号可观测 | 成功口径模糊 | 当前 change |
| A3 | Success 3: 证据包格式冻结 | 检查 evidence 要求 | 后续 smoke 证据可复用 | 证据路径与格式清楚 | 证据结构不统一 | 当前 change |
| A4 | Failure 1: 不把临时脚本冒充正式 smoke | 对照 scope | 临时诊断与正式 smoke 分离 | 正式入口唯一 | 临时脚本继续混用 | 当前 change |
| A5 | Failure 2: 不越界做完整 adapter | 对照 scope | 只冻结 baseline | 范围收敛 | 任务范围失控 | 当前 change |
| A6 | Boundary 1: Topic 1 收尾但不替代 Topic 2-4 | 对照 roadmap | 交接边界清楚 | Topic 交接清晰 | Topic 分层混乱 | 当前 change |

## 六、证据清单 / Evidence

| # | 证据类型 | 路径/链接 | 说明 |
| --- | --- | --- | --- |
| 1 | Topic 1 上游结论 | `/D:/Nautilus/nautilus_ctp_adapter/docs/changes_topic/roadmap/nautilus_adapter/ctp-live-connectivity/README.md` | baseline 必须建立在 Topic 1 已冻结结论上 |
| 2 | 当前 change 输出证据 | 当前 change bundle 内新增 | 正式 smoke 入口、成功信号、证据格式 |
| 3 | docs index 回写 | `/D:/Nautilus/nautilus_ctp_adapter/docs/README.md` | 用于声明后续 topic 的统一 smoke 入口 |

## 七、最终结论 / Final Verdict

- **结论**：⬜ 待执行
- **建议**：等待 C4 完成后再冻结正式 baseline，避免用未收敛的 TD readiness 提前定口径
- **说明**：当前 verdict 只代表 C5 bundle 状态，不代表 Topic 1 已自动收尾。

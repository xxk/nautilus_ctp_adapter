# Merged Reconciliation Policy Evidence
...
5. 当前 findings 为：
   ...
   `no_current_session_callbacks -> evidence_only`

## 关键结论

1. 当前仓内已经有正式的 `TD merged reconciliation policy` 自动输出层。
2. 真实 `025292` 的 live 结果已经证明：本轮 merged truth 需要保留历史 callback 边界，同时因 `available_ratio` 与 `margin_ratio` 命中阈值，结论应升级为 `manual_review_required`。
3. 本 evidence 只基于真实 live smoke，不依赖 test、mock、fake 结果宣告通过。

## 原始证据

1. [merged_reconciliation_policy_20260402.log](./merged_reconciliation_policy_20260402.log)

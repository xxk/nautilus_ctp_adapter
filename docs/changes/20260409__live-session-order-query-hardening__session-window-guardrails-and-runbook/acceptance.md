...
| E4 | 关键证据已留存 | ⬜ | 交易时段、非交易时段、guardrail 失败至少各有一份 evidence | 当前 change 证据文件 |
| E5 | 正式验收不依赖 mock 或 test | ⬜ | 正式场景必须使用真实 CTP、真实账户、真实配置路径 | 当前 change 证据文件 |
...
| # | 场景 | 执行 | 结论 | 阻塞 | 证据/备注 |
| --- | --- | :---: | :---: | :---: | --- |
| A1 | Success 1: 交易时段 TD preflight ready | ⬜ | ⬜ | 是 | `ctp_td_order_truth_smoke.py` |
| A2 | Success 2: `c2609` 一手 live order dev loop 走通 | ⬜ | ⬜ | 是 | `ctp_order_lifecycle_smoke.py --live-send` |
| A3 | Success 3: 非交易时段 query snapshot 走通 | ⬜ | ⬜ | 是 | `ctp_query_adapter_smoke.py` |
| A4 | Failure 1: 非交易时段误用 live-send 被明确阻断 | ⬜ | ⬜ | 是 | session-window runbook / 脚本失败语义 |
| A5 | Failure 2: guardrail 越界订单在触达 TD 前被拒绝 | ⬜ | ⬜ | 是 | `ctp_order_lifecycle_smoke.py` 失败证据 |
| A6 | Boundary 1: 空仓或无持仓不被误判成查询失败 | ⬜ | ⬜ | 否 | `ctp_position_query_smoke.py` |
...
| --- | --- | :---: | :---: | :---: | --- |
| 本地 real-account live config 已准备好 | 配置 | 是 | 是 | ⬜ | 从 [cfgs/ctp.live.example.json](/D:/Nautilus/nautilus_ctp_adapter/cfgs/ctp.live.example.json) 复制到忽略目录 `cfgs/local/ctp.live.025292.local.json`，不得直接改 tracked 模板 |
...
| # | 证据类型 | 路径/链接 | 说明 |
| --- | --- | --- | --- |
| 1 | 交易时段 preflight | `./evidence_a1_trade_window_preflight.md` | A1 的 ready/login/order truth 证据 |
| 2 | 交易时段 live order loop | `./evidence_a2_trade_window_live_order.md` | A2 的 live-send 结构化结果 |
| 3 | 非交易时段 query snapshot | `./evidence_a3_offhours_query_snapshot.md` | A3 的 account/position 只读快照 |
| 4 | 非交易时段误用 live-send 阻断 | `./evidence_a4_offhours_live_send_block.md` | A4 的失败口径证据 |
| 5 | guardrail 失败语义 | `./evidence_a5_guardrail_violation.md` | A5 的 precheck 拒绝证据 |
| 6 | 空仓边界 | `./evidence_a6_empty_position_boundary.md` | A6 的 no_positions 非失败证据 |
...
- **执行人**：—
- **建议**：暂不建议宣告通过
- **说明**：当前已冻结真实场景验收矩阵，后续实现与证据回填必须围绕 A1-A6 推进。
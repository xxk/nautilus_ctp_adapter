...
| 顺序 | 建议 change-id | 作用 |
| --- | --- | --- |
| C1 | `20260402__nautilus-live-marketdata__marketdata-runtime-event-contract` | 冻结 runtime 到 adapter 的市场数据事件 contract |
| C2 | `20260402__nautilus-live-marketdata__live-data-client-bootstrap` | 建立最小 `LiveDataClient` 主线 |
| C3 | `20260402__nautilus-live-marketdata__subscription-restore-and-batching` | 收口恢复、批量 drain 和节流语义 |
| C4 | `20260402__nautilus-live-marketdata__nautilus-marketdata-smoke-baseline` | 冻结行情 smoke 入口与证据格式 |

## 五、AI-TASK-QUEUE

**当前状态**：等待 Topic 2 完成后激活。

- [ ] 创建 `C1` child change bundle
- [ ] 完成 `C1 -> C2 -> C3 -> C4`
- [ ] 回写 mainline roadmap 与 Topic 4 进入条件

**激活规则**：当 Topic 2 标记为 completed 时，先创建 `C1`，再将本 topic 标记为 `in_progress`。
